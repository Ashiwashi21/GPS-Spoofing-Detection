...
if accs:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, "cnn_cm_aggregate.png"))
    plt.close()
...
plt.title("Conv1D Weight Distribution")
...
plt.title("Conv1D Kernels - Mean Abs Weight")
...
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(RESULTS_DIR, "cnn_train_val_accuracy.png"))
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(RESULTS_DIR, "cnn_train_val_loss.png"))
plt.close()
