...
def generate_gps_path(n_rows, timestamps, start_lat, start_lon, base_alt=300.0, avg_speed=1.3):
    ...
    for i in range(1, n_rows):
        ...
        # Inject random bursts and turns
        if np.random.rand() < 0.05:
            heading += math.pi / 2  # sharp turn
            speed *= np.random.uniform(2.0, 4.0)

        heading += math.radians(np.random.normal(0, 5))
        dist = speed * dt
        ...
        # Add micro-jitter
        lat[i] = lat[i - 1] + dlat + np.random.normal(0, 0.0002)
        lon[i] = lon[i - 1] + dlon + np.random.normal(0, 0.0002)
        alt[i] = base_alt + np.random.normal(0, 1.5)
        speed_knots[i] = speed / 0.514444
    return lat, lon, alt, speed_knots

# ---------- IMU AUGMENTATION ----------

def augment_sequence(seq, n_augments=15):
    augmented = []
    for _ in range(n_augments):
        noise = np.random.normal(0, [0.3, 0.3, 0.3, 1.5, 1.5, 1.5, 0.2], size=seq.shape)
        drift = np.linspace(0, np.random.uniform(0.5, 2.0), seq.shape[1])
...
def main():
    ...
    print(f"?? Generated {len(full_df)} hard-mode rows -> {out_path}")
    print(full_df.head())
