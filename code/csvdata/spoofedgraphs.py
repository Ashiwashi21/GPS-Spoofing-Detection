...
ax = axes[0]
ax.plot(dx_normal, dy_normal, color="green", linewidth=1.5, alpha=0.6, label="path")
...
for col in imu_cols:
    if col in spoofed.columns:
        spoofed[col] = pd.to_numeric(spoofed[col], errors="coerce")

plt.figure(figsize=(12,6))
for col in ["accel_x","accel_y","accel_z"]:
    if col in spoofed.columns:
        plt.plot(spoofed[col], label=col)
plt.title("Accelerometer Data Over Time (Spoofed)")
plt.xlabel("Sample")
...
for col in ["gyro_x","gyro_y","gyro_z"]:
    if col in spoofed.columns:
        plt.plot(spoofed[col], label=col)
plt.title("Gyroscope Data Over Time (Spoofed)")
plt.xlabel("Sample")
plt.ylabel("Angular Velocity (deg/s)")
plt.legend()
plt.show()

if "temp" in spoofed.columns:
    plt.figure(figsize=(12,4))
    plt.plot(spoofed["temp"], label="Temperature")
    plt.title("IMU Temperature Over Time (Spoofed)")
    plt.xlabel("Sample")
    plt.ylabel("Temp (�C)")
    plt.legend()
    plt.show()
